function ranovatbl = nWayRM_poly(data,varargin)
...
if ~mod(N,2)
    ...
    rm = fitrm(tbl,['V1-V',num2str(condNum),'~1'],'WithinDesign',[1:condNum]');
    %[ranovatbl] = ranova(rm,'WithinModel',factName{1});
    ranovatbl = anova(rm,'WithinModel','orthogonalcontrasts');
