function edf = select_saccades_all(edf,set)
...
for ii = 1:edf.events.num_trial
    ...
    % do not select those with artifacts
     ind = find(edf.events.sac.trial == ii & edf.events.sac.msg_srt == 7 ...
         & edf.events.sac.artifact == 0);
%          & edf.events.sac.amp >= 1 & edf.events.sac.dur >= 50 & ...
     ...
     % euclidean distance
     clear dist
     for jj = 1:4 % for each target
         dist(:,jj) = sqrt((x_tar(jj) - x_end).^2 + (y_tar(jj) - y_end).^2);
     end
