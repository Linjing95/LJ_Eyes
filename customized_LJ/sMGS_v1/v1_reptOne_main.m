...
data_dir = 'C:\Users\lj104\Documents\Linjing_Research\thesis_data\images\Behavior\v1_1\sMGS\p3_1\';
...
edf = select_saccades_one(edf,set);
...
for ii = 1:4
    ind = find(p_ord==ii);
p_err_new(:,ii) = [p_err(ind);nan(30-length(ind),1)];
a_err_new(:,ii) = [a_err(ind);nan(30-length(ind),1)];
n_sac_new(:,ii) = [n_sac(ind);nan(30-length(ind),1)];
...
%% Rearranged based on reporting order
p_ind = edf.cal.primary_sac_ind;
a_ind = edf.cal.acc_sac_ind;

for ii = 1:96
[p_ind_sort(ii,:),ip] = sort(p_ind(ii,:));
p_err_sort(ii,:) = p_err(ii,ip);
IP(ii,:) = ip;

[a_ind_sort(ii,:),ia] = sort(a_ind(ii,:));
a_err_sort(ii,:) = a_err(ii,ia);

[n_sac_sort(ii,:),in] = sort(n_sac(ii,:));
end

figure(1);clf
boxWithDots2(p_err_sort,{'1ST', '2ND', '3RD', '4TH'},[],[])
xlabel('Reporting Order')
ylabel('Primary Saccade Error (dva)')

figure(2);clf
boxWithDots2(a_err_sort,{'1ST', '2ND', '3RD', '4TH'},[],[])
xlabel('Reporting Order')
ylabel('Smallest Saccade Error (dva)')

figure(3);clf
boxWithDots2(n_sac_sort,{'1ST', '2ND', '3RD', '4TH'},[],[])
xlabel('Reporting Order')
ylabel('Number of Saccades <= 3.5 dva')

figure(4);clf
boxWithDots2(IP,{'1ST', '2ND', '3RD', '4TH'},[],[])
xlabel('Encoding Order')
ylabel('Reporting Order')
...
hold on
for jj = 1
scatter(tm(indm(ii,jj+1)),edf.param.tarx_deg(ii,jj),40,[0, 0.4470, 0.7410])
scatter(tm(indm(ii,jj+1)),edf.param.tary_deg(ii,jj),40,[0.8500, 0.3250, 0.0980])
hold on
end
% for jj = 1:4
%     
