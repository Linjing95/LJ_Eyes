...
%data_dir = 'C:\Users\lj104\Documents\Linjing_Research\thesis_data\images\fMRI\sf4_1\fMRI_Eye\sMGS_v1_TR990_run3\pf4_1\';
%data_dir = 'C:\Users\lj104\Documents\Linjing_Research\thesis_data\images\fMRI\pf4_1\Behavior_Eye\sMGS_v1_TR990_run2\p4_1_prac\'
...
% pause
set = v3_setting();
set.eye = 2; % for this subject, right eye
...
edf = get_screen_size(edf,1,143.5,43,35,1024,768);
...
edf = detect_epoch_v3_fMRI(edf,set);
...
edf = select_saccades_one_v3_fMRI(edf,set); % select saccades
...
% probe_order
p_ord = edf.param.probe_order(2:17);
...
for ii = 1:4
    ind = find(p_ord==ii);
p_err_new(:,ii) = [p_err(ind);nan(16-length(ind),1)];
a_err_new(:,ii) = [a_err(ind);nan(16-length(ind),1)];
n_sac_new(:,ii) = [n_sac(ind);nan(16-length(ind),1)];
p_rt_new(:,ii) = [p_rt(ind);nan(16-length(ind),1)];
...
%% Effects of quadrant
p_quad = edf.param.probe_quad(2:17);
...
p_img = edf.param.probe_img(2:17);
...
line([tm(indm(ii,1)),tm(indm(ii,size(indm,2)))],[edf.param.tarx_deg(ii+1,jj),edf.param.tarx_deg(ii+1,jj)],'LineStyle','--','Color',[0, 0.4470, 0.7410])
line([tm(indm(ii,1)),tm(indm(ii,size(indm,2)))],[edf.param.tary_deg(ii+1,jj),edf.param.tary_deg(ii+1,jj)],'LineStyle','--','Color',[0.8500, 0.3250, 0.0980])
