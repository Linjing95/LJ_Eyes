function edf = detect_epoch_v3_fMRI(edf,set)
...
for ii = 1:length(msg) % for each message
    ...
    ind = find(ind==1);
    %edf.default_events.Messages.info(ind)
    if ii == 1|ii == 14  % TRIAL ID
        ind(1) = []; 
    if max(edf.samples.trial) == 18
        ind(end) = [];
    end
    elseif ii == 2 % Fixation
        if edf.samples.trial(end) == 18
            ind(end) = [];
        end
    elseif ii == 10|ii == 13 % delay or ITI
        ind = endsWith(edf.default_events.Messages.info,msg{ii});
        ind = find(ind==1);
    end
...
% for ii = 1:14
% ntr(ii) = sum(msgText==ii)
% end
ntr = sum(msgText==length(msg)); % only count finished trials
...
edf.events.num_trial = ntr; edf.events.msg.txt = []; edf.events.msg.time = [];
if mod(length(msgText),length(msg))
    for ii = 1:length(msg)
temp = find(msgText==ii); temp = temp(1:ntr);
edf.events.msg.txt = [edf.events.msg.txt msgText(temp)];
edf.events.msg.time = [edf.events.msg.time msgTime(temp)];
    end
else
