...
file_dirs = dir([data_dir 'VGS*\p*']); % run*\p*
...
% Load parameter file
edf = load_param_v3_vgs(edf);

% First, we need to detect different trials and task epochs
edf = detect_epoch_v3_vgs(edf,set);
...
edf = select_saccades_v3_vgs(edf,set); % select saccades
