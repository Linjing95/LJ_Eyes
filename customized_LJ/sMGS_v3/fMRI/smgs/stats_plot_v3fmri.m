...
data_dir = 'C:\Users\lj104\Documents\Linjing_Research\thesis_data\images\fMRI\pilot\'; %'C:\Users\lj104\Documents\Linjing_Research\thesis_data\images\Behavior\v3_short\';

% We want to process each data folder separately
file_dirs = dir([data_dir 'f*\Eye\smgs*\f*']);

%%
clear subs runs
for ii = 1:length(file_dirs)
    subs{ii} = file_dirs(ii).name;
    runs(ii) = str2num(erase(file_dirs(ii).folder,[data_dir subs{ii} '\Eye\smgs']));
end
...
% Stats analysis
[ranovatbl,Mrm,mau] = nWayRM(data_nnan,'Run',{'run1','run2','run3','run4'});
...
figure(3);clf
boxWithDots3(data_nnan,{'Q1','Q2','Q3','Q4'},1,repmat([0 0.4470 0.7410],4,1),[],[]) % 
xlabel('Run')
ylabel('Percentage of trials excluded (%)')
ylim([0 80])
box off
...
final_sind = 1:7;%find(~isnan(data_clean(:,1))); %
...
ev_plot = [500 1000 1000 1000 500 10000 1000];
...
[ranovatbl,Mrm,mau,eps] = nWayRM(temp,'Order',{'First','Second','Third','Fourth'});%'Task',{'Quadrant Cue','Order Cue'},
...
[ranovatbl,Mrm,mau,eps] = nWayRM(temp,'Quadrant',{'First','Second','Third','Fourth'});%'Task',{'Quadrant Cue','Order Cue'},
...
figure(1);clf
boxWithDots3(temp,{'URV','ULV','LLV','LRV'},0,...
   [repmat([0 0.4470 0.7410],4,1);],p_val,comp) % repmat([0.8500 0.3250 0.0980],4,1)
...
[ranovatbl,Mrm,mau,eps] = nWayRM(temp,'Image',{'First','Second','Third','Fourth'});%'Task',{'Quadrant Cue','Order Cue'},
