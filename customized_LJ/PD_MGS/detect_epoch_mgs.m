function edf = detect_epoch_mgs(edf,set)
...
if ii == 1 | ii == length(msg)
    ind = find(startsWith(edf.default_events.Messages.info,'TRIALID'));
else
ind = find(startsWith(edf.default_events.Messages.info,'-') ...
    & endsWith(edf.default_events.Messages.info,set.msg{ii}) ...
    & ~contains(edf.default_events.Messages.info,'session'));
length(ind);
end
%     ind = find(ind==1);
...
% clc
% for jj = 1:length(ind)
% edf.default_events.Messages.info{ind(jj)}
% pause
% end
