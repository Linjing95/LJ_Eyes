...
%data_dir = 'C:\Users\lj104\Documents\Linjing_Research\Obeso\LJ_Eyes\mgs_healthy_ignacio_study\mgs_healthy_ignacio_study\'; % Behav_v3_long\sMGS\
...
% end
%%
for ff = 1:length(file_dirs) %ff_ind%
...
cd([[],file_dir]) % make sure you entered the data folder
edf_file = dir('*.edf');
...
% output folder)
plot_artifact(edf,[],file_dir,out_dir,set);
...
writetable(tbl,[[],file_dir,out_dir,'trackloss_id',edf.ID,'.csv']);
clear tbl
...
fprintf('\n');
fprintf('saving .mat file...');
save([[],file_dir,out_dir,filename,'_step3']);
...
% figures)
plot_timecourse_clean(edf,[],file_dir,out_dir,set);
...
fprintf('\n');
fprintf('saving .mat file...');
save([[],file_dir,out_dir,filename,'_step5']);
...
% fprintf('\n');
% fprintf('saving .mat file...');
% save([[],file_dir,out_dir,filename,'_step6']);
% 
% % % Timing
