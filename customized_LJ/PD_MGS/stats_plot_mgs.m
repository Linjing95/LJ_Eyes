%% This is for PD task, not vswmda tasks?????
...
data_dir = 'C:\Users\lj104\Documents\Linjing_Research\Obeso\LJ_Eyes\pd_Dec2022\mgs\'; % Behav_v3_long\sMGS\

% We want to process each data folder separately
% file_dirs = dir([data_dir 'sMGS*\p*']);

% We want to process each data folder separately
file_dirs = dir([data_dir]); % run*\p*
% remove all files (isdir property is 0)
file_dirs = file_dirs([file_dirs(:).isdir]);
% remove '.' and '..' 
file_dirs = file_dirs(~ismember({file_dirs(:).name},{'.','..'}));

%%
clear subs runs
for ii = 1:length(file_dirs)
    subs{ii} = file_dirs(ii).name;
    runs(ii) = 1;%str2num(erase(file_dirs(ii).folder,[data_dir]));
end
...
for ii = 1:length(subs_id)
    ...
    for jj = 1:length(ind)
    file = dir([file_dirs(ind(jj)).folder '\' file_dirs(ind(jj)).name '\result\*step5.mat']);
...
for ii = 1:length(subs_id)
    for jj = 1:length(runs_id)
        ...
        calib_avg(ii,jj) = max(data{ii,jj}.calib.avg_error);
        calib_max(ii,jj) = max(data{ii,jj}.calib.max_error);
        calib_pt(ii,jj) = str2num(data{ii,jj}.calib.pattern(3:end));
...
qa.calib_avg = calib_avg;
qa.calib_max = calib_max;
qa.calib_pt = calib_pt;
%%
...
tbl = table(subs_id',qa.perc_mean,qa.nblink_mean,qa.calib_avg,qa.calib_max,...
    qa.calib_pt,'VariableNames',{'ID','% Sample Loss','Avg. No. Blinks Per Trial',...
    'calib_avg','calib_max','calib_pt'});

writetable(tbl,'data_quality.csv')

%% More detailed data quality report
...
% Stats analysis
[ranovatbl,Mrm,mau,eps] = nWayRM(data_nnan,'Task',{'Quadrant Cue','Order Cue'},'Order',{'First','Second','Third','Fourth'});
...
% 13, iti
% 14, end
eye = 1;%contains(data{ii,jj}.record.eye,'left');
x = data{ii,jj}.samples.x_deg_clean(:,eye); 
y = data{ii,jj}.samples.y_deg_clean(:,eye);

% all the following are actual data points, not the timing
% event start
ev_srt = data{ii,jj}.events.msg.ind_srt(:,[2 3 4 5 6 7]); 
...
ev_srt_new = ev_srt;
%ev_srt_new(:,1) = [];
ev_srt_new(:,1) = ev_srt_new(:,2)-500*double(data{ii,jj}.record.sample_rate)/1000;
%ev_srt_new(:,end) = [];

% plotted event timing (data points, not actual timing)
ev_plot = [500 500 1700 2600 2500 1000]*double(data{ii,jj}.record.sample_rate)/1000;
...
ylabel('Vertical Eye Position (dva)')
title(subs_id{ii})
saveas(figure(1),[subs_id{ii},'_map.jpg'])
%axis off
% %%
% % plot gaze path
...
tm = (1:max(run_len))/double(data{ii,jj}.record.sample_rate);
figure(2);clf
...
line([ev_plot(kk) ev_plot(kk)]/double(data{ii,jj}.record.sample_rate),[-12 12],'Color','k','LineStyle','--');
hold on
end
box off
xlabel('Time (sec)')
ylabel('Horizontal(Blue)/Vertical(Orange) Eye Position (dva)')
title(subs_id{ii})
saveas(figure(2),[subs_id{ii},'_path.jpg'])
end
%% average gaze path
